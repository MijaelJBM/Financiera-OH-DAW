...
def listar_creditos(conn: Connection, pkcliente: int) -> list[dict]:
    ...
    Si el crédito es nuevo y no tiene historial mensual, jala el monto real aprobado 
    directamente desde su solicitud.
    """
    sql = text(
        """
        SELECT cr.codcuentacredito,
               COALESCE(fa.fechadesembolsocredito, sol.fechasolicitudcredito, CURRENT_DATE) AS fecha_desembolso,
               
               -- Si no hay cierre de cartera, usamos el monto real aprobado de la solicitud
               COALESCE(fa.montosaldocapital, sol.montoaprobadocredito, sol.montosolicitudcredito, 0.0) AS saldo_capital,
               COALESCE(fa.montosaldocliente, sol.montoaprobadocredito, sol.montosolicitudcredito, 0.0) AS pago_pendiente,
        ...
        -- Hacemos un LEFT JOIN con dsolicitud para rescatar los montos reales del préstamo original
        LEFT JOIN dsolicitud sol
          ON sol.pkcliente = cr.pkcliente AND sol.pksolicitudestado = 4
